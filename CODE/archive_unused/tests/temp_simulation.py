...
print("--- Voronoi Fickian Diffusion (robust version) ---")

# Reproducible seed
SEED = 1

# 1) Build Voronoi network using helper
from NETWORK_GENERATION_FINAL import build_voronoi_network
domain = (0.0025, 0.0025, 0.0025)
density = 1e13
...
try:
    _ = op.utils.check_network_health(pn)
except Exception:
    pass
...
if missing:
    raise RuntimeError(f"Required network arrays missing: {missing}")

# 2) Create phase + physics
...
if np.any(bad_td):
    ...
    if pos.size > 0:
        ...
        mean_pos_td = 1e-9
    # Use a conservative fraction of the mean positive throat diameter as the
    # replacement to avoid introducing unrealistically tiny throats.
...
if np.any(bad_tl):
    ...
    if posl.size > 0:
        ...
        # fallback: estimate using euclidean distances between connected pores
...
if bad_g:
    ...
    # As an extra robust measure, recompute conductances for ALL throats
    # using a physics-based formula so the network becomes connected.
    try:
        ...
        # Enforce a small positive floor to avoid zero conductances which create
        # isolated pores and degenerate linear systems during solve.
        eps_g = 1e-12
...
# Use labeled boundary pore sets if available
...
print(f"Applied BCs: C={C_in} on {len(inlet_pores)} inlet pores; C={C_out} on {len(outlet_pores)} outlet pores")
...
try:
    ...
    # show conductances for throats touching an example outlet pore
...
if isolated.size > 0:
    print(f'Pinning {isolated.size} isolated pores to C_in to avoid singular matrix')
    fd.set_value_BC(pores=isolated, values=C_in, mode='overwrite')
...
L = domain[0]
A = domain[1] * domain[2]
D_eff = rate_in * L / (A * (C_in - C_out)) if (C_in - C_out) != 0 else 0.0
...
try:
    ...
    ax.set_axis_off()
    fig.tight_layout()
    out = 'fickian_voronoi_concentration.png'
